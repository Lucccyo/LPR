...
export class DndApiRepository implements ApiRepository {
  ...
  public async getRaces(): Promise<Race[]> {
    ...
    for (const result of data.results) {
      ...
      if (race_data.languages) {
        const language_urls: string[] = race_data.languages.map(
          (lang: { url: string }) => `https://www.dnd5eapi.co${lang.url}`,
        );
      ...
      // traits
      const trait_url: string[] = race_data.traits.map(
        (trait: { url: string }) => `https://www.dnd5eapi.co${trait.url}`,
      );
  ...
  public async getAll(): Promise<FetchReturn> {
    const [classes, alignments, races] = await Promise.all([this.getClasses(), this.getAlignments(), this.getRaces()]);
    return {
      classes,
      alignments,
      races,
